<?php 
    ...
    $hasDish = mysqli_fetch_array(mysqli_query($DBConnect, "SELECT dishID FROM dish WHERE dishName = '$dishName' AND status = 'Active';"));

    if (($hasDish) == 0) {
        mysqli_query($DBConnect, "INSERT INTO Pending_Dish(dishName, type, dateCreated, img) 
        VALUES ('$dishName', 'New', NOW(), '$filePathInDatabase');");
